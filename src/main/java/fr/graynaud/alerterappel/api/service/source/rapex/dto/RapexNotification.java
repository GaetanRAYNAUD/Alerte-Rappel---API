...
public record RapexNotification(
    ...
    private static final String RECALL_SHEET_URL_PREFIX = "https://ec.europa.eu/safety-gate-alerts/screen/webReport/alertDetail/";

    public Alert toAlert(Map<String, String> translations) {
        AlertMetadataSource source = new AlertMetadataSource(
                RapexService.SOURCE_NAME, this.id,
                this.reference != null ? RECALL_SHEET_URL_PREFIX + this.reference : null,
                OffsetDateTime.now(),
                null
        );
        AlertMetadata metadata = new AlertMetadata(List.of(source), null);

        List<String> risks = this.risk != null && this.risk.riskType() != null
                             ? this.risk.riskType().stream().map(r -> translate(r.key(), translations)).toList()
                             : null;
        ...
        AlertMedia media = mapMedia();

        return new Alert(
                metadata,
                this.reference != null ? this.reference.toUpperCase() : null,
    ...
    private AlertProduct mapProduct(Map<String, String> translations) {
        ...
        String brand = this.product.brands() != null && !this.product.brands().isEmpty()
                       ? this.product.brands().getFirst().brand()
                       : null;

        List<String> barcodes = this.product.barcodes() != null
                                ? this.product.barcodes().stream().map(RapexBarcode::barcode).toList()
                                : null;

        List<String> batchNumbers = this.product.batchNumbers() != null
                                    ? this.product.batchNumbers().stream().map(RapexBatchNumber::batchNumber).toList()
                                    : null;

        List<String> modelRefs = this.product.modelTypes() != null
                                 ? this.product.modelTypes().stream().map(RapexModelType::modelType).toList()
                                 : null;

        return new AlertProduct(
                this.product.nameSpecific(),
                version != null ? version.name() : null,
                version != null ? version.description() : null,
                brand,
                null,
                this.product.productCategory() != null ? translate(this.product.productCategory().key(), translations) : null,
                optionToBoolean(this.product.isCounterfeit()),
                barcodes != null && !barcodes.isEmpty() ? barcodes : null,
                batchNumbers != null && !batchNumbers.isEmpty() ? batchNumbers : null,
                modelRefs != null && !modelRefs.isEmpty() ? modelRefs : null,
                version != null ? version.packageDescription() : null,
                null
        );
    }

    private AlertCommercialization mapCommercialization(Map<String, String> translations) {
        String originCountry = this.traceability != null && this.traceability.countryOrigin() != null
                               ? this.traceability.countryOrigin().name()
                               : null;

        String alertCountry = this.country != null ? this.country.name() : null;

        List<String> reacting = this.reactingCountries != null
                                ? this.reactingCountries.stream()
        ...
        return new AlertCommercialization(
                originCountry,
                alertCountry,
                reacting != null && !reacting.isEmpty() ? reacting : null,
                soldOnline,
                null,
                null,
                null
    ...
    private AlertMeasures mapMeasures(Map<String, String> translations) {
        ...
        List<AlertMeasureItem> measureItems = this.measureTaken.measures() != null
                                              ? this.measureTaken.measures().stream().map(m -> mapMeasureItem(m, translations)).toList()
                                              : null;

        List<AlertCompanyRecall> companyRecalls = this.measureTaken.companyRecalls() != null
                                                  ? this.measureTaken.companyRecalls().stream().map(r -> mapCompanyRecall(r, translations)).toList()
                                                  : null;

        return new AlertMeasures(
                recallOnline,
                measureItems != null && !measureItems.isEmpty() ? measureItems : null,
                companyRecalls != null && !companyRecalls.isEmpty() ? companyRecalls : null,
                null,
                null,
                null
        );
    ...
    private AlertMeasureItem mapMeasureItem(RapexMeasure measure, Map<String, String> translations) {
        RapexMeasureVersion version = measure.versions() != null && !measure.versions().isEmpty()
                                      ? measure.versions().getFirst()
                                      : null;

        return new AlertMeasureItem(
                measure.measureCategory() != null ? translate(measure.measureCategory().key(), translations) : null,
    ...
    private AlertCompanyRecall mapCompanyRecall(RapexCompanyRecall recall, Map<String, String> translations) {
        return new AlertCompanyRecall(
                recall.link(),
                recall.language() != null ? translate(recall.language().key(), translations) : null
        );
    }

    private AlertMedia mapMedia() {
        List<String> photos = this.product != null && this.product.photos() != null
                              ? this.product.photos().stream().map(p -> PHOTO_URL_PREFIX + p.id()).toList()
                              : null;

        String recallSheetUrl = this.reference != null ? RECALL_SHEET_URL_PREFIX + this.reference : null;

        return new AlertMedia(
                photos != null && !photos.isEmpty() ? photos : null,
                recallSheetUrl
        );
    }

    private RapexProductVersion firstProductVersion() {
        return this.product != null && this.product.versions() != null && !this.product.versions().isEmpty()
               ? this.product.versions().getFirst()
               : null;
    }

    private RapexRiskVersion firstRiskVersion() {
        return this.risk != null && this.risk.versions() != null && !this.risk.versions().isEmpty()
               ? this.risk.versions().getFirst()
               : null;
    }
